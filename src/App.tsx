...
export function App() {
  ...
  const scienceText: Record<Tier, string> = {
    safe: `카페인은 뇌의 아데노신 수용체를 차단해 각성 효과를 제공하지만, 취침 시점에 혈중 농도가 낮아지면 아데노신이 정상적으로 작동해 수면을 촉진합니다. 이로 인해 입면 시간이 짧아지고, 깊은 수면(서파수면) 비율이 유지되어 다음 날 컨디션이 최적화됩니다. 또한 적정 수준 이하에서는 카페인 대사 산물인 파라잔틴이 신경을 안정시켜 정신적 명료성을 제공할 수 있습니다.`,
    caution: `카페인이 아데노신 수용체를 일부 계속 막고 있는 상태라, 잠들기까지 걸리는 시간이 평소보다 늘어나고 얕은 수면 비중이 높아질 수 있어요. 특히 새벽에 자주 깨는 '단편수면'으로 이어지기 쉽습니다.`,
    danger: `이 정도 농도의 카페인은 아데노신 수용체를 강하게 차단해 졸음 신호 자체를 억제해요. 잠들더라도 깊은 수면과 렘수면 비율이 크게 줄어들어, 자고 일어나도 피로가 풀리지 않는 얕은 수면에 쉽습니다.`
  };
    safe: `카페인은 뇌의 아데노신 수용체를 차단해 각성 효과를 제공하지만, 취침 시점에 혈중 농도가 낮아지면 아데노신이 정상적으로 작동해 수면을 촉진합니다. 이로 인해 입면 시간이 짧아지고, 깊은 수면(서파수면) 비율이 유지되어 다음 날 컨디션이 최적화됩니다. 또한 적정 수준 이하에서는 카페인 대사 산물인 파라잔틴이 신경을 안정시켜 정신적 명료성을 제공할 수 있습니다.`,
    caution: `카페인이 아데노신 수용체를 일부 계속 막고 있는 상태라, 잠들기까지 걸리는 시간이 평소보다 늘어나고 얕은 수면 비중이 높아질 수 있어요. 특히 새벽에 자주 깨는 '단편수면'으로 이어지기 쉽습니다.`,
    danger: `이 정도 농도의 카페인은 아데노신 수용체를 강하게 차단해 졸음 신호 자체를 억제해요. 잠들더라도 깊은 수면과 렘수면 비율이 크게 줄어들어, 자고 일어나도 피로가 풀리지 않는 얕은 수면에 쉽습니다.`
  ...
  const actionSteps: Record<Tier, string[]> = {
    safe: [
      '현재 패턴이 최적입니다 — 같은 섭취 시각을 지속해 몸의 리듬을 유지하세요.',
      '아침·오전 중 섭취량을 약간 늘려도 수면에 영향을 주지 않는 여유가 있습니다.',
      '취침 1시간 전 스마트폰 사용을 최소화하고, 블루라이트 차단 모드를 활용해 수면의 질을 향상시키세요.',
      '수분 섭취를 충분히 유지하면 카페인 대사가 촉진되어 잔존량이 빠르게 감소합니다.',
      '정기적인 가벼운 스트레칭은 혈액 순환을 돕고, 카페인 대사 효소 활동을 지원합니다.'
    ],
    caution: [
      `마지막 섭취 시각을 ${lastDrinkOk} 이전으로 앞당겨서 안전 마진을 확보하세요.`,
      '오후 카페인 섭취를 절반 용량으로 줄이거나 디카페인으로 대체해 대사 부담을 감소시키세요.',
      '취침 3시간 전부터는 물, 허브차, 혹은 무카페인 음료로 전환해 카페인 잔존량을 낮추세요.',
      '잠자리 전 따뜻한 샤워나 명상으로 신체를 이완시켜, 남은 카페인이 수면에 미치는 영향을 최소화합니다.',
      '가능하면 저녁 시간대에 카페인 섭취를 완전히 중단하고, 대신 단백질이 풍부한 스낵을 섭취해 에너지 레벨을 유지하세요.'
    ],
    danger: [
      `마지막 섭취를 ${lastDrinkOk} 이전으로 앞당겨 급격히 카페인 잔존량을 줄이세요.`,
      '오후 낮잠을 피하고, 저녁에는 완전 금주(디카페인 포함) 원칙을 적용해 교감신경 과부하를 방지합니다.',
      '취침 전 가벼운 스트레칭, 요가, 혹은 미온수 샤워로 근육을 이완하고, 코르티솔 수치를 낮추세요.',
      '다음날 아침에 충분한 수분과 함께 비타민 B군 보충제로 카페인 대사 효소를 활성화하세요.',
      '잠들기 전 차분한 독서나 호흡 운동을 통해 심박수를 낮추고, 카페인으로 인한 각성 효과를 상쇄합니다.'
    ],
  };

  const handleShare = async () => {
    const text = `☕ 카페인 반감기 진단: ${sleepStatus.label}\n🕒 현재 시각: ${nowStr}\n🔬 취침 시 잔존 카페인: ${Math.round(atBed)}mg (안전 기준 ${SAFE_SLEEP_THRESHOLD}mg 이하 권장)\n⏰ 완전 소멸 예상 시각: ${clearTime}\n💡 맞춤 조언: ${actionSteps[tier][0]}\n${typeof window !== 'undefined' ? window.location.href : ''}`;
    // 최우선 공유 API 사용, 실패 시 클립보드 복사
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({ title: 'CAFFEINE HALF-LIFE LAB', text, url: window.location.href });
        return;
      } catch {
        // 공유 취소 혹은 오류, fallback to clipboard
      }
    }
